#!/bin/sh
# OpenLGTV BCM 0.5.0-beta2 installation script v.1.95 by xeros
...
echo "" | tee -a $log
date 2>&1 | tee -a $log
echo "OpenLGTV BCM 0.5.0-beta2 installation script for $platform platform by xeros" | tee -a $log
...
if [ "$no_install" = "1" ]
then
    echo "To install OpenLGTV BCM you need to use \"$0 install\" command."
    echo "More usage info: \"$0 --help\""
    exit 0
fi
