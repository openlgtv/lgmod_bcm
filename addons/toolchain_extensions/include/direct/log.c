           ...
           const char *filename )
{
     ...
     //backup log file by yducky --->
     int status = -1;
     int result = -1;
     char sys_cmd_string[256];
     sprintf(sys_cmd_string, "touch %s && mv %s %s.old",filename, filename, filename);

     status = system(sys_cmd_string);
	if (-1 != status)
	{
